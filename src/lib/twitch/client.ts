// Twitch OAuth device-code flow + token validate/refresh.
// Ported from the parent subathon_timer desktop app (proven, unchanged logic).
...
): Promise<TwitchDeviceCodeFlow> {
  const response = await fetch(DEVICE_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: toFormBody({ client_id: clientId, scopes: scopes.join(' ') }),
  })
...
export async function pollDeviceCode(
  ...
  const payload = await parseJson(response)
  if (response.ok) {
    return { kind: 'success', tokens: mapTokenExchange(payload) }
  }
