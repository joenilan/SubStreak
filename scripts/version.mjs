...
switch (command) {
  ...
  case 'sync':
    ...
    logSuccess(`Synced SubStreak version ${currentVersion}.`)
    break
  case 'patch':
  case 'minor':
  case 'major': {
    ...
    logSuccess(`Bumped SubStreak version to ${nextVersion}.`)
    break
  }
  case 'set': {
    if (!nextVersionArg) {
    ...
    logSuccess(`Set SubStreak version to ${nextVersionArg}.`)
    break
  }
  default:
...
function runCheck(expectedVersion) {
  ...
  if (mismatches.length > 0) {
    ...
    fail(`SubStreak versions are out of sync.\nExpected: ${expectedVersion}\n${lines.join('\n')}`)
  }

  logSuccess(`SubStreak versions are in sync at ${expectedVersion}.`)
}
...
function runNotesCheck(expectedVersion) {
  ...
  if (missing.length > 0) {
    ...
    fail(`SubStreak release notes are missing version ${expectedVersion}.\n${lines.join('\n')}`)
  }

  logSuccess(`SubStreak release notes include ${expectedVersion}.`)
}
