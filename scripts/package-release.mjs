...
const releaseBinary = resolve(appRoot, 'src-tauri', 'target', 'release', 'substreak.exe')
const releaseRoot = resolve(appRoot, 'release', 'windows')
const releaseSlug = 'substreak'
...
writeFileSync(
  portableReadmePath,
  [
    `SubStreak Portable ${version}`,
    '',
    `Run ${portableExeName} directly.`,
    'The app keeps your Twitch session and streak progress in your user app-data folder.',
    '',
    'Contents:',
    `- ${portableExeName}`,
    `- ${portableReadmeName}`,
...
console.log(`Packaged Windows release artifacts for SubStreak ${version}:`)
...
function findBundleArtifact(directory, extension, activeVersion) {
  ...
  if (versionedEntries.length === 1) return versionedEntries[0]
